...
PUB_ID = 'IOT55' # 본인 아이피 마지막주소
BROKER = '210.119.12.55' # 본인 아이피
PORT = 1883
TOPIC = 'smarthome/55/topic'  # publish/subscribe에서 사용할 토픽(핵심)
...
try:
    ...
    while True:
        ...
        rain = random.randint(0, 1) # [Fake] 비, 실제로는 센서에서 값을 받아옴, 0은 맑음. 1은 1ㅣ
        detect = random.randint(0, 1)   # [Fake] 인체센서, 실제로는 센서에서 값을 받아옴
        photo = random.randint(50, 255) # [Fake] 포토센서, 255쪽이 어두움 센싱

        COUNT += 1
        ## 센싱데이터를 json형태로 변경
        ## OrderedDict로 먼저구성. 순서가 있는 딕셔너리 객체
        raw_data = OrderedDict()
        raw_data['PUB_ID'] = PUB_ID
        raw_data['COUNT'] = COUNT
        raw_data['SENSING_DT'] = currtime.strftime(f'%Y-%m-%d %H:%M:%S') # C# 'yyyy-MM-dd'
